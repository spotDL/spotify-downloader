...
async def test_get_download_status_not_found(authenticated_client: AsyncClient):
    ...
    with (
        patch("spotdl.api.v1.download.get_settings") as mock_settings,
        patch("spotdl.api.v1.download.get_download_service") as mock_manager,
    ):
        mock_settings.return_value.downloads_enabled = True
        ...
        response = await authenticated_client.get("/api/v1/download/status/nonexistent-id")
    ...
    with (
        ...
        mock_downloads = [
            DownloadProgress(
                ...
                error=None,
                created_at=datetime.now(UTC),
                completed_at=datetime.now(UTC) if i % 2 == 1 else None,
            )
            for i in range(5)
        ]
...
async def test_cancel_download_not_found(authenticated_client: AsyncClient):
    ...
    with (
        ...
        response = await authenticated_client.post("/api/v1/download/cancel/nonexistent")
...
async def test_cancel_download_success(authenticated_client: AsyncClient):
    ...
    with (
        ...
        response = await authenticated_client.post("/api/v1/download/cancel/test-cancel")
