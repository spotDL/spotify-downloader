...
files = (join(dir, file) for dir, folders, files in walk(rootdir) for file in files)
...
for eachFile in files:
    ...
    try:
        ...
        if lineCount > limit:
            print(
                "| %-111s | %-4d | %-4d | ⚠ |"
                % (eachFile[-111:], lineCount, lineCount - limit)
            )
        else:
            if not onlyErrors:
                print(
...
print(
    "TOTAL %-4d\t\tSUCCESS %-4d\t\tFAIL %-4d\t\tSKIPPED %-4d"
