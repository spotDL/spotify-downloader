...
from spotdl_cli.matching.constants import FORBIDDEN_WORDS
...
def check_common_word(song: Song, result: DownloadResult) -> bool:
    """
...
def check_forbidden_words(song: Song, result: DownloadResult) -> tuple[bool, list[str]]:
    """
...
def create_match_strings(
    song: Song, result: DownloadResult, search_query: str | None = None
) -> tuple[str, str]:
    """
...
def calc_main_artist_match(song: Song, result: DownloadResult) -> float:
    """
    ...
    _sorted_song_artists, sorted_result_artists = based_sort(song_artists, result_artists)
...
def calc_artists_match(song: Song, result: DownloadResult) -> float:
    """
...
def artists_match_fixup1(song: Song, result: DownloadResult, score: float) -> float:
    """
...
def artists_match_fixup2(
    song: Song, result: DownloadResult, score: float, search_query: str | None = None
) -> float:
    """
...
def artists_match_fixup3(song: Song, result: DownloadResult, score: float) -> float:
    """
...
def calc_time_match(song: Song, result: DownloadResult) -> float:
    """
...
def calc_album_match(song: Song, result: DownloadResult) -> float:
    """
    ...
    Returns:
        Match score from 0 to 100
    """
    if not result.album_name or not song.album_name:
        return 0.0
