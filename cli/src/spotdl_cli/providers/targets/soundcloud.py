...
class SoundCloudTargetProvider(TargetProvider):
    """
    ...
    def _extract_hydration_data(self, html: str) -> list[dict[str, Any]]:
        """Extract hydration data from SoundCloud page."""
        match = re.search(r"__sc_hydration\s*=\s*(\[.*?\]);", html, re.DOTALL)
        if match:
            try:
    ...
    def _track_to_result(self, track: dict[str, Any]) -> DownloadResult:
        """Convert SoundCloud track data to DownloadResult object."""
        # Get artist info
        user = track.get("user", {})
    ...
    async def search(self, song: Song, limit: int = 10) -> list[DownloadResult]:
        ...
        try:
            ...
            hydration_data = self._extract_hydration_data(response.text)
