...
class downloadTracker():
    ...
    def load_tracking_file(self, trackingFile: str) -> None:
        ...
        self.adjustingFactor = len(self.songObjList)
    ...
    def notify_download_progress(self, stream, chunk, bytes_remaining) -> None:
        ...
        downloadedBit = len(chunk) / size * 80
        
        self.progressBar.update(downloadedBit/self.adjustingFactor)
        print(self.progressBar.n)
    ...
    def update_status(self):
        cDownloads  = self.count['downloading']
        cConverting = self.count['converting']
        cEmbedding  = self.count['embedding']
...
def download_song(displayManager: downloadTracker, songObj: songObj) -> None:
    ...
    command = 'ffmpeg -v quiet -y -i "%s" -ar 44100 -af loudnorm=i=-17 "%s"'
    ...
    displayManager.notify_download_complete(songObj)