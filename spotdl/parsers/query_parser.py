...
def parse_query(
    query: List[str], format, use_youtube, generate_m3u, lyrics_provider, threads, path_template
) -> List[SongObject]:
    """
    ...
    for request in query:
        ...
        songs_list.extend(
            parse_request(
                request, format, use_youtube, generate_m3u, lyrics_provider, threads, path_template
            )
        )
...
def parse_request(
    ...
    if (
        ...
        song_list = song_gatherer.from_album(
            request, output_format, use_youtube, lyrics_provider, generate_m3u, threads, path_template
        )
