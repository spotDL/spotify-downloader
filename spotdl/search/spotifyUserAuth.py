    ...
    class CallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path.startswith("/callback?"):
                try:
                    parsedRequest = parse_qs(self.path[10:], strict_parsing=True, errors="strict")
                    if parsedRequest.get("error"):
                        ...
                        output['data'] = (
                            parsedRequest.get("code")[0],
                            parsedRequest.get("state")[0]
                        )
                except Exception:
...
def _refresh_access_token(inst):
    ...
    while(inst._active):
        ...
        authString = base64.b64encode(
            bytes(f"{inst._clientId}:{inst._clientSecret}", 'utf-8')
        ).decode('utf-8')
        ...
        if newToken.get("error"):
            raise Exception(
                f"Could not refresh Access Token: {newToken.get('error')}"
            )
