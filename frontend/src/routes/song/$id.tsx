...
import type { AudioFeatures, CreateMetadataReportRequest } from "@/types";
...
function SongPage() {
  ...
  const { data: matchesData, isLoading: matchesLoading } = useMatchesForSong(id);
  const discoverMatches = useDiscoverMatchesMutation(id);
  ...
  const displayMetadata = useMemo(() => {
    ...
    const data = activeSnapshot.data as Record<string, any>;
  ...
  return (
    <div className="space-y-8 animate-slide-up">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Left Column - Lyrics & Content */}
        <div className="lg:col-span-2 space-y-6">

          {/* Audio Matches Section */}
          <Card variant="bordered" className="overflow-hidden">
...
function MatchRow({ match, isAuthenticated }: { match: import("@/types").Match; isAuthenticated: boolean }) {
